...
void CreateDrawBuffers(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(draw_normals[0]) * draw_normals.size(), &draw_normals[0], GL_STATIC_DRAW);
	glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, 4 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(2);

	glBindBuffer(GL_ARRAY_BUFFER, draw_VBO[3]);
	glBufferData(GL_ARRAY_BUFFER, sizeof(glm::vec2) * draw_texture_coordinates.size(), &draw_texture_coordinates[0], GL_STATIC_DRAW);
...
{	
	...
	for (int i = 0; i < Tori.size(); i++) { //render each entity individually, so a seperate transformation can be applied to it.
		...
		for (int j = 0; j < Tori[i]->mesh.size(); j++) {
			...
			draw_texture_coordinates.push_back(Tori[i]->texture_coordinates[j]);
		};
		...
		std::cout << Tori[i]->textureId << std::endl;
		PerspectiveTextureLightShader.SetUniform("texId", Tori[i]->textureId);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, Tori[i]->textureId);
		//draw it
		CreateDrawBuffers();
